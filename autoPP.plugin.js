...
class AutoPP {
	load() {
		...
		Status.headers = {
			'Authorization': Status.authToken,
...
const Status = {
	Check: async (id) => {
		try {
			if(!id)BdApi.showToast('please provide your username or id');
			// const resp = await fetch(`${Status.osuUrl}/?id=${id}`);
			const resp = await fetch(`${Status.osuUrl}/?id=${id}`);
			const data = await resp.json();
			const pp = `PP: ${data.pp}`;

			const test = await fetch(Status.url,{headers: Status.headers}).then(async res=>await res.json())
			const oldpp = test.custom_status.text;
			if(test.custom_status===null){
				console.log(test.custom_status===null);
				Status.Set(pp);
				return;
			}
